...
function getOptions(opts) {
  var options = {
    appBaseURL: process.env.NODE_ENV === 'production' ? 'https://echo.learnersguild.org' : 'http://echo.learnersguild.dev'
  };
  if (opts.token) {
    if (!opts.handle) {
      var _gitConfig$sync = _gitConfig2.default.sync(),
          handle = _gitConfig$sync.github.user;
...
function printResult(result) {
  var log = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : console.info;
...
if (!module.parent) {
  ...
  var help = opts.help,
      _ = opts._,
      filteredOpts = _objectWithoutProperties(opts, ['help', '_']); // eslint-disable-line no-unused-vars
